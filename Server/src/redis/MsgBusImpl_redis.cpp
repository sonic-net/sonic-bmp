 ...
 ********************************************************************/
MsgBusImpl_redis::MsgBusImpl_redis(Logger *logPtr, Config *cfg) {
    logger = logPtr;
    this->cfg = cfg;
}
...
void MsgBusImpl_redis::update_Peer(obj_bgp_peer &peer, obj_peer_up_event *up, obj_peer_down_event *down, peer_action_code code) {
    ...
    redis.WriteBMPTable(BMP_TABLE_NEI, keys, fieldValues);
}
...
void MsgBusImpl_redis::update_unicastPrefix(obj_bgp_peer &peer, std::vector<obj_rib> &rib,
    ...
    fieldValues.reserved(30);
    std::vector<std::pair<std::string, std::string>> addFieldValues;
    ...
    for (size_t i = 0; i < rib.size(); i++) {
        std::vector<std::string> keys;
        keys.reserved(10);
        keys.emplace_back(rib[i].prefix);
        keys.emplace_back("/");
        keys.emplace_back(std::to_string(rib[i].prefix_len));
        ...
        switch (code) {
            ...
            case UNICAST_PREFIX_ACTION_ADD:
            {
                ...
                if(peer.isAdjIn)
                {
                    redis.WriteBMPTable(BMP_TABLE_RIB_IN, keys, addFieldValues);
                }
                else
                {
                    redis.WriteBMPTable(BMP_TABLE_RIB_OUT, keys, addFieldValues);
                }
            }
                break;
